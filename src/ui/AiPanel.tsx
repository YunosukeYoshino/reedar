...
export function RunStatus({ state }: { state: RunState }) {
  ...
  return <span className={`run-status status-${state.status}`}><Icon size={12} className={state.status === "running" ? "spin" : ""} />{state.status === "running" && state.phase === "fetching" ? "本文を取得中" : labels[state.status]}</span>;
}
...
export function AiPanel({ article, agent, setAgent, conversation, connections, act, perform, close }: Props) {
  ...
  return <aside className="ai-panel" aria-label="AIとの会話"><header className="ai-heading"><div><Bot size={17} /><h2>AIと読む</h2></div><button className="icon-button" aria-label="AIパネルを閉じる" onClick={close}><X size={16} /></button></header>
    ...
    {conversation?.source.origin ? <p className="source-caption">{conversation.source.origin === "web" ? `リンク先本文 · ${conversation.source.text.length.toLocaleString()}文字` : "フィード本文のみ"}{conversation.source.fetchError ? <span>{conversation.source.fetchError}</span> : null}</p> : <p className="source-caption">質問時にリンク先の本文を取得します。</p>}
    <div className="conversation-scroll" ref={scroll} onScroll={() => { const element = scroll.current; if (element) follow.current = element.scrollHeight - element.scrollTop - element.clientHeight < 80; }}>
      {!conversation?.messages.length ? <div className="ai-welcome"><Bot size={25} strokeWidth={1.4} /><h3>この記事を、もう少し深く。</h3><p>要点をつかむ。背景を知る。<br />気になった一文について尋ねる。</p><div className="suggestions">{["この記事を日本語で要約して", "この記事の背景を説明して", "この記事から何がわかる？"].map((text) => <button key={text} onClick={() => void send(text)} disabled={sending}>{text}<ArrowUp size={13} /></button>)}</div></div> : conversation.messages.map((message) => <div key={message.id} className={`message message-${message.role}`}><div className="message-label">{message.role === "user" ? "あなた" : message.model === "gpt-5.3-codex-spark" ? "Codex · Spark" : agentName[agent]}{message.role === "assistant" ? <><RunStatus state={message.state} />{message.sourceOrigin === "feed" ? <span>フィード本文のみ</span> : null}</> : null}</div>{message.role === "user" ? <p>{message.text}</p> : <><div className="markdown"><MarkdownText text={message.text} />{!message.text && message.state.status === "running" ? <span className="thinking">記事を読んでいます…</span> : null}</div>{message.state.status === "failed" || message.state.status === "waiting" ? <div className="run-notice" role="status">{message.state.status === "failed" ? message.state.error : message.state.reason}{message.state.status === "waiting" ? <div className="toolbar-group"><button className="text-button" commandfor="connections-dialog" command="show-modal">接続を確認</button><button className="text-button" aria-label="確認待ちを中止" onClick={() => perform({ type: "chat.stop", conversationId: conversation.id })}>中止</button></div> : null}</div> : null}</>}</div>)}
    </div>
